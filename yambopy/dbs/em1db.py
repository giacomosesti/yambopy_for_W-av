...
class YamboScreeningDB(object):
    """
    Class to handle screening databases from Yambo
    
    This reads the databases ``ndb.em1s*`` or ``ndb.pp*`` or ``ndb.Xmpa*``
    There :math:`√v(q,g1) \chi_{g1,g2} (q,\omega) √v(q,g2)` is stored.
    ...
    """
    def __init__(self,save='.',em1='.',filename='ndb.pp',db1='ns.db1',do_not_read_cutoff=False):
        self.save = save
        self.pp = em1
        ...
        #read em1s database
        if not os.path.isfile("%s/%s"%(self.pp,self.filename)): 
            if os.path.isfile("%s/%s"%(self.pp,'ndb.em1s')): self.filename = 'ndb.em1s' # check for em1s instead of ppa
            if os.path.isfile("%s/%s"%(self.pp,'ndb.Xmpa')): self.filename = 'ndb.Xmpa' # check for Xmpa instead of ppa
            else: raise FileNotFoundError("File %s not found."%self.filename)

        try:
            database = Dataset("%s/%s"%(self.pp,self.filename), 'r')
        except:
            raise IOError("Error opening %s/%s in YamboScreeningDB"%(self.save,self.filename))
        ...
        for iQ in range(self.nqpoints):
            if not os.path.isfile("%s/%s_fragment_%d"%(self.pp,self.filename,iQ+1)): read_fragments=False
    ...
    def readDBs(self):
        """
        Read the yambo databases, we access the fragment 1 to set the number of frequencies
        """
        #open database 1
        filename = "%s/%s_fragment_%d"%(self.pp,self.filename,1)
        try:
           database = Dataset(filename)
        except:
           print("warning: failed to read %s"%filename)
        self.freq=database.variables['X_Q_%d'%(1)].shape[0]

        #create database to hold all the X data
        self.X = np.zeros([self.nqpoints,self.freq,self.size,self.size],dtype=np.complex64)
        for nq in range(self.nqpoints):

            #open database for each k-point
            filename = "%s/%s_fragment_%d"%(self.pp,self.filename,nq+1)
            try:
                database = Dataset(filename)
            except:
                print("warning: failed to read %s"%filename)

            # screening means we have all the frequencies
            re = database.variables['X_Q_%d'%(nq+1)][...,0]
            im = database.variables['X_Q_%d'%(nq+1)][...,1]
    ...
    def writeeps(self,filename='em1.dat',ng1=0,ng2=0,volume=False):
        """
    ...
    def get_Coulomb(self):
        ...
        if self.cutoff!='none' and not self.no_cutoff:
            ...
            if os.path.isfile('%s/ndb.cutoff'%self.pp):
                try:
                    database = Dataset("%s/ndb.cutoff"%self.pp, 'r')
                ...
                print("[WARNING] Cutoff %s was used but ndb.cutoff not found in %s. Make sure this is fine for what you want!"%(self.cutoff,self.pp))
